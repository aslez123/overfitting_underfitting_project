...
def create_wellfitted_model(dataType):
    if dataType == DataType.MNIST:
        input_shape = 784
    elif dataType == DataType.IMDB:
        input_shape = 10000
    else:
        input_shape = 3072
...
def create_underfitted_model(dataType):
    if dataType == DataType.MNIST:
        input_shape = 784
    elif dataType == DataType.IMDB:
        input_shape = 10000
    ...
    model = keras.Sequential([
        keras.layers.Dense(4, activation=tf.nn.relu, kernel_regularizer=regularizers.l2(0.1), input_shape=(input_shape,)),
...
def create_overfitted_model(dataType):
    if dataType == DataType.MNIST:
        input_shape = 784
    elif dataType == DataType.IMDB:
        input_shape = 10000
    ...
    model = keras.models.Sequential([
        keras.layers.Dense(512, activation=tf.nn.relu, input_shape=(input_shape,)),
...
def train_model(model, batch_size, epochs, model_name, X_train, Y_train):
    weights_path = (
        MODEL_PATH_PREFIX + model_name+ "/"+ model_name + "_" + str(epochs)+"_{epoch}.hdf5"
    )
    ...
    if model_name == 'wellfitted_model':

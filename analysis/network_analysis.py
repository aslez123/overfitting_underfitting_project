...
def get_activations_overfitted_model(model, data, size):
    layer_indices = [0, 1, 2]
    layer_output = K.function(
        [model.layers[0].input], [model.layers[i].output for i in layer_indices]
    )
...
def get_activations_underfitted_model(model, data, size):
...
def get_activations_wellfitted_model(model, data, size):
    layer_indices = [0, 1, 2, 3, 4]
    layer_output = K.function(
        [model.layers[0].input], [model.layers[i].output for i in layer_indices]
    )
...
def compare_projections(datatype, model_name, n_layer, label, size=2000):
    def get_model_and_activations():
        ...
        elif "overfitted_model" in model_name:
            load_data_func = load_data_my_model
            create_model_func = create_overfitted_model
            get_activations_func = get_activations_overfitted_model
        elif "underfitted_model" in model_name:
            load_data_func = load_data_my_model
            create_model_func = create_underfitted_model
            get_activations_func = get_activations_underfitted_model
        elif "wellfitted_model" in model_name:
            load_data_func = load_data_my_model
            create_model_func = create_wellfitted_model
            get_activations_func = get_activations_wellfitted_model
...
#dla mnist już nie zadziała bo musiałabym zwiększyć lcizbę labelów do 10
...
def compare_discriminative_map(datatype, model_name, n_layer, size):
    if "mlp" in model_name:
        ...
        X_train, Y_train, X_test, Y_test = load_data_my_model(datatype)
        model_bt = create_overfitted_model(datatype)
        layer_bt = get_activations_overfitted_model(model_bt, X_test, size)[n_layer - 1]
        
        model_at = create_overfitted_model(datatype)
        load_weights_from_file(model_at, model_name, 100, 100)
        layer_at = get_activations_overfitted_model(model_at, X_test, size)[n_layer - 1]

    elif "underfitted_model" in model_name:
        X_train, Y_train, X_test, Y_test = load_data_my_model(datatype)
        model_bt = create_underfitted_model(datatype)
        layer_bt = get_activations_underfitted_model(model_bt, X_test, size)[n_layer - 1]
        
        model_at = create_underfitted_model(datatype)
        load_weights_from_file(model_at, model_name, 100, 100)
        layer_at = get_activations_underfitted_model(model_at, X_test, size)[n_layer - 1]

    elif "wellfitted_model" in model_name:
        X_train, Y_train, X_test, Y_test = load_data_my_model(datatype)
        model_bt = create_wellfitted_model(datatype)
        layer_bt = get_activations_wellfitted_model(model_bt, X_test, size)[n_layer - 1]
        
        model_at = create_wellfitted_model(datatype)
        load_weights_from_file(model_at, model_name, 100, 100)
        layer_at = get_activations_wellfitted_model(model_at, X_test, size)[n_layer - 1]
...
def show_seq_projections(datatype, model_name, n_layer, epoch, size):
    ...
    points_transformed, targets = show_tsne(model_name, epoch, layer_at, Y_test, init=None)
...
def inter_layer_evolution(points_lst, targets):
    ...
    points_by_target = [[] for _ in range(2)]
    for n in range(2):
...
def inter_epoch_evolution(points_lst, targets):
    points_by_digit = [[] for _ in range(2)]
    for n in range(2):
...
def show_tsne_epoch_trace(model_name, datatype):
    ...
    if "mlp" in model_name:
        ...
        _, _, X_test, Y_test = load_data_underfitted_model(datatype)
        create_model = lambda: create_my_model(datatype)
        get_activations = lambda m, d: get_activations_underfitted_model(m, d, 2000)[-1]
    elif "overfitted_model" in model_name:
        _, _, X_test, Y_test = load_data_my_model(datatype)
        create_model = lambda: create_overfitted_model(datatype)
        get_activations = lambda m, d: get_activations_overfitted_model(m, d, 2000)[-1]
    elif "wellfitted_model" in model_name:
        _, _, X_test, Y_test = load_data_my_model(datatype)
        create_model = lambda: create_wellfitted_model(datatype)
        get_activations = lambda m, d: get_activations_wellfitted_model(m, d, 2000)[-1]
